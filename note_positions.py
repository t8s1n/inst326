...
def get_fret(target, string):
    ...
    string_note = notes[string]
    target_note = notes[target]

    fret = (target_note - string_note) % 12
...
def parse_args(arglist):
    ...
    parser.add_argument("target", help="the note whose position the user wants to find")
    parser.add_argument("strings", nargs='+', help="a list of one or more notes of open strings for which the user "
                                                   "wishes to find the position of the target note")

    return parser.parse_args(arglist)


# > Takes command-line arguments from sys.argv, excluding sys.argv[0] (the path to your script)
# > Passes the command-line arguments to your parse_args() function and stores the result in a variable
# > Calls your get_frets() function, passing in values you parsed with your parse_args() function,
# and stores the result in a variable
# > Prints a message for each key/value pair in the result of get_frets()
